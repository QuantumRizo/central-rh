...
export function Profile({ session, employeeId, viewerEmployeeId, isAdmin = false, onBack, backLabel = 'Volver a perfiles', onEvaluations, onChangePassword }: { session: Session; employeeId: string; viewerEmployeeId: string; isAdmin?: boolean; onBack?: () => void; backLabel?: string; onEvaluations: () => void; onChangePassword?: () => void }) {
  ...
  return <div className="profile-page">
    <div className="page-header"><div>{onBack && <button className="back-link" type="button" onClick={onBack}><ArrowLeft size={17} /> {backLabel}</button>}<p className="eyebrow">{isOwn ? 'MI ESPACIO' : 'EQUIPO'}</p><h1>{isOwn ? 'Mi perfil' : 'Perfil del colaborador'}</h1><p>{isOwn ? 'Tu información laboral y un resumen de actividad.' : 'Datos laborales y actividad registrada en el sistema.'}</p></div></div>
    ...
    {busy && !employee ? <div className="evaluation-loading"><LoaderCircle className="spin" size={20} /> Cargando perfil…</div> : employee && <>
      ...
      <TimesheetCalendar employeeId={employeeId} selectedDate={localDate(today)} refreshKey={0} />
