...
import { Camera, ClipboardCheck, Clock3, LoaderCircle, Mail, UserRound } from 'lucide-react';
...
export function Profile({ session, employeeId, onEvaluations }: { session: Session; employeeId: string; onEvaluations: () => void }) {
  ...
  const uploadPhoto = async (file?: File) => {
    ...
    if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) { setError('Usa una imagen JPG, PNG o WebP.'); return; }
    if (file.size > 2 * 1024 * 1024) { setError('La foto debe pesar menos de 2 MB.'); return; }
    setUploading(true); setError(''); setMessage('');
    try {
      const extension = file.type === 'image/jpeg' ? 'jpg' : file.type === 'image/png' ? 'png' : 'webp';
      const path = `${session.user.id}/avatar-${Date.now()}.${extension}`;
      const { error: uploadError } = await sb.storage.from('profile-photos').upload(path, file, { contentType: file.type, upsert: false });
  ...
  return <div className="profile-page">
    <div className="profile-heading"><p className="eyebrow">MI ESPACIO</p><h1>Mi perfil</h1><p>Tu información laboral y un resumen de actividad.</p></div>
    ...
    {busy && !employee ? <div className="evaluation-loading"><LoaderCircle className="spin" size={20} /> Cargando perfil…</div> : employee && <>
      <section className="profile-hero">
        ...
        <div className="profile-photo-action"><input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp" aria-label="Seleccionar foto de perfil" onChange={(event) => void uploadPhoto(event.target.files?.[0])} hidden /><button className="secondary" type="button" disabled={uploading} onClick={() => inputRef.current?.click()}><Camera size={17} /> {uploading ? 'Subiendo…' : photoUrl ? 'Cambiar foto' : 'Agregar foto'}</button><small>JPG, PNG o WebP · máximo 2 MB</small></div>
      </section>
      <div className="profile-grid">
        <section className="profile-card"><div className="profile-card-title"><UserRound size={19} /><h2>Datos del sistema</h2></div><dl className="profile-details"><div><dt>Nombre</dt><dd>{employee.full_name}</dd></div><div><dt>Correo</dt><dd><Mail size={14} /> {employee.email || session.user.email || 'Sin registrar'}</dd></div><div><dt>Puesto</dt><dd>{employee.position || 'Por asignar'}</dd></div><div><dt>Área</dt><dd>{employee.department || 'Por asignar'}</dd></div><div><dt>Estado</dt><dd>{employee.status}</dd></div><div><dt>En la plataforma desde</dt><dd>{dateLabel.format(new Date(employee.created_at))}</dd></div></dl></section>
