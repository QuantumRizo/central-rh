...
export function ProfilesDirectory({ session, viewerEmployeeId, onEvaluations }: {
  ...
  const groups = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      const primary = order === 'surname' ? compare(surname(a.full_name), surname(b.full_name)) : order === 'position' ? compare(a.position, b.position) : compare(a.full_name, b.full_name);
      return primary || compare(a.full_name, b.full_name);
    });
    const grouped = new Map<string, Person[]>();
    for (const person of filtered) {
      const heading = order === 'position' ? person.position || 'Por asignar' : (order === 'surname' ? surname(person.full_name) : person.full_name).charAt(0).toLocaleUpperCase('es-MX') || '#';
  ...
  return <div className="profiles-directory">
    ...
    <section className="profiles-filters" aria-label="Buscar y ordenar perfiles">
      ...
      <label>Ordenar por<select value={order} onChange={(event) => setOrder(event.target.value as Order)}><option value="surname">Último apellido</option><option value="position">Puesto</option><option value="name">Nombre</option></select></label>
