...
begin
  ...
  if v_attendance = 'worked' then
    if v_mode is null or v_entry is null or v_exit is null or v_exit <= v_entry then raise exception 'Horario inválido'; end if;
    ...
    v_mode := null; v_entry := null; v_exit := null; v_entries := '[]'::jsonb;
  end if;
  insert into public.timesheets(employee_id,work_date,attendance,mode,entry_time,exit_time,position_snapshot,updated_at)
    values (v_employee_id,v_date,v_attendance,v_mode,v_entry,v_exit,v_position,now())
    on conflict (employee_id,work_date) do update set attendance=excluded.attendance,mode=excluded.mode,entry_time=excluded.entry_time,exit_time=excluded.exit_time,position_snapshot=excluded.position_snapshot,updated_at=now()
...
$$;
revoke insert, update, delete on public.timesheets, public.timesheet_entries from authenticated;
